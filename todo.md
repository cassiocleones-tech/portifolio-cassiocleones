# Project TODO

- [x] Copiar logo para diretório public do projeto
- [x] Configurar identidade visual (cores NR-12: azul #003366, laranja #FF6600)
- [x] Criar seção hero com apresentação profissional
- [x] Criar seção de serviços (4 serviços principais)
- [x] Criar seção de projetos anteriores (editável pelo usuário)
- [x] Adicionar informações de contato (email e telefone)
- [x] Implementar design responsivo
- [x] Adicionar animações e micro-interações
- [x] Testar navegação e layout
- [x] Criar checkpoint do projeto
- [x] Criar documento PDF profissional do portfólio
- [x] Adicionar informação sobre parceria Gautica no website e PDF
- [x] Criar 6 imagens de exemplo para galeria de projetos
- [x] Implementar galeria de imagens na seção de Projetos Anteriores
- [x] Alterar nome de Industra Safe para Cassio Cleones em todo o site e PDF
